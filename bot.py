...
# Webhook sozlamalari (o'zingiz o'zgartiring!)
WEBHOOK_HOST = "https://pychatbot-3vzv.onrender.com"      # Masalan: https://bot.example.com
...
async def process_btn_text(message: types.Message, state: FSMContext):
    ...
    await message.answer("🔗 Endi tugma bosilganda ochiladigan **URL** kiriting:", parse_mode="Markdown")
...
async def process_btn_url(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "✅ Sozlamalar saqlandi!\n\n"
        "Endi botni Telegram Business → Chatbots bo'limida ulang.",
        parse_mode="Markdown"
    )
...
async def on_business_connect(connection: types.BusinessConnection):
    if connection.is_enabled:
        ...
        logging.info(f"Biznes ulandi: {connection.id} | User: {connection.user.id}")
    else:
